...
class M365_LM_Database {
    ...
    /**
     * Cache for combined license types to avoid repeated queries and keep a single definition.
     */
    private static $combined_license_types_cache = null;
    ...
    public static function get_combined_license_types() {
        ...
        if (self::$combined_license_types_cache !== null) {
            return self::$combined_license_types_cache;
        }
        ...
        if (!$licenses_exists) {
            self::$combined_license_types_cache = $types_exists ? self::get_license_types() : array();
            return self::$combined_license_types_cache;
        }
        ...
        foreach ($types as $type) {
            $key = !empty($type->sku) ? strtolower($type->sku) : ('plan_' . md5($type->name));
            $indexed[$key] = $type;
        }
        ...
        foreach ($distinct_licenses as $license) {
            ...
            $normalized_sku = strtolower($license->sku);

            if (!isset($indexed[$normalized_sku])) {
                $license->name             = $license->plan_name;
                $license->display_name     = $license->plan_name;
                $license->show_in_main     = 1;
                $indexed[$normalized_sku] = $license;
            } else {
                $existing = $indexed[$normalized_sku];
                $existing->name = $existing->name ?: $license->plan_name;
                if (empty($existing->display_name)) {
                    $existing->display_name = $license->plan_name;
                }
                ...
                $indexed[$normalized_sku] = $existing;
            }
        }

        self::$combined_license_types_cache = array_values($indexed);
        return self::$combined_license_types_cache;
    }
     ...
     * קבלת סוגי רישיונות ממוזגים מהטבלה הייעודית ומהרישיונות שנשמרו בפועל
     */
    public static function get_license_types_combined() {
        global $wpdb;
    ...
    /**
     * תאימות לאחור: עטיפה לשם הישן כדי למנוע שגיאות redeclare בסביבות בהן הוגדר בעבר.
     */
    public static function get_combined_license_types() {
        return self::get_license_types_combined();
    }
     ...
     * קבלת סוגי רישיונות ממוזגים מהטבלה הייעודית ומהרישיונות שנשמרו בפועל
     *
     * שם הפונקציה נשמר כ-get_combined_license_types כדי למנוע שגיאות על קריאות קיימות.
     */
    public static function get_combined_license_types() {
        global $wpdb;
    ...
    /**
     * תאימות לאחור: עטיפה לשם החדש עבור קוד שעבר להשתמש ב-get_license_types_combined.
     */
    public static function get_license_types_combined() {
        return self::get_combined_license_types();
    }
    ...
    /**
     * ליבה פנימית מאוחדת לבניית רשימת סוגי הרישיונות עם Cache סטטי.
     *
     * השמות ההיסטוריים get_combined_license_types / get_license_types_combined
     * נפתרים דרך __callStatic כדי להימנע מהצהרות כפולות בעת שדרוגים חלקיים.
     */
    private static function build_combined_license_types() {
        global $wpdb;
    ...
    /**
     * תאימות לאחור: ניתוב דינמי לשמות הפונקציות ההיסטוריים ללא הצהרה כפולה.
     */
    public static function __callStatic($name, $arguments) {
        $normalized = strtolower($name);
        if (in_array($normalized, array('get_combined_license_types', 'get_license_types_combined'), true)) {
            return self::build_combined_license_types();
        }
    ...
    /**
     * שם חדש פנימי ברור לשימוש בקוד העדכני, מבלי להתנגש בהצהרות ישנות.
     */
    public static function get_license_types_combined() {
        return self::build_combined_license_types();
    }
